...
void DetectorConstruction::DefineMaterials() {
    ...
    G4cout << "[WRAP] light scale " << lysoScale << " -> "
...
G4VPhysicalVolume* DetectorConstruction::Construct() {
    ...
    G4cout << "[WRAP] beamline: MCP=" << useMCP << " triggers=" << useTrig
           << " PbGlass=" << usePbGlass
    ...
    G4cout << "[WRAP] corner capillary type: " << (eType ? "E (full-length WLS, energy)"
    ...
    // layers, acting as a spacer against LYSO/W optical contact
    // (2401.01747 sec 2: "inserted between successive layers ... to avoid
    // optical absorption between LYSO:Ce and W surfaces"). Neither paper
    // describes a reflector on the OUTSIDE of the assembled module. So: does
    // adding one buy light, and what does that light cost in timing?
    //
    // WHY THE AIR GAP IS THE WHOLE BALLGAME. LYSO has n = 1.81, so at a bare
    // LYSO/air side face every photon beyond the critical angle (~33.6 deg) is
    // already totally internally reflected -- for FREE, at 100%. A wrap can
    // only ever recover the ESCAPE CONE. Worse, if the wrap is placed in
    // optical CONTACT (gap = 0) it destroys that TIR and replaces a perfect
    // mirror with a 98% one, which can LOSE light on net. Real Tyvek is not
    // index-matched -- it sits against the surface with microscopic air voids
    ...
    G4double wrapThk  = tyvekThick;    // mm, one 0.008" foil thickness
    if (const char* s = std::getenv("RADWRAP_REFLECTIVITY")) {
        double v = std::atof(s); if (v >= 0. && v <= 1.) wrapRefl = v;
    }
    if (const char* s = std::getenv("RADWRAP_GAP_MM")) {
        double v = std::atof(s); if (v >= 0.) wrapGap = v;
    }
    if (const char* s = std::getenv("RADWRAP_THICK_MM")) {
        double v = std::atof(s); if (v > 0.) wrapThk = v;
    }
    // Tyvek is a DIFFUSE (Lambertian) reflector; ESR/VM2000 and aluminized
    // mylar are SPECULAR. In Geant4's UNIFIED model the difference is real and
    // easy to get backwards: with finish=ground and NO specular constants set,
    // the Lambertian remainder is 1 (fully diffuse) -- that is the correct
    // Tyvek behaviour. Specular therefore has to be asked for EXPLICITLY, both
    // via finish=polished and via SPECULARSPIKECONSTANT=1.
    bool wrapSpecular = false;
    if (const char* s = std::getenv("RADWRAP_FINISH"))
        wrapSpecular = (std::string(s) == "specular");

    if (wrapSides || wrapEnds) {
        auto wrapSurf = new G4OpticalSurface("Wrap");
        wrapSurf->SetType(dielectric_metal);
        wrapSurf->SetModel(unified);
        wrapSurf->SetFinish(wrapSpecular ? polished : ground);
        const std::vector<G4double> wE = {1.55*eV, 3.54*eV};
        auto wMPT = new G4MaterialPropertiesTable();
        wMPT->AddProperty("REFLECTIVITY", wE,
                          std::vector<G4double>{wrapRefl, wrapRefl});
        if (wrapSpecular)
            wMPT->AddProperty("SPECULARSPIKECONSTANT", wE,
                              std::vector<G4double>{1.0, 1.0});
        wrapSurf->SetMaterialPropertiesTable(wMPT);

        auto wrapVis = new G4VisAttributes(G4Colour(1.0,1.0,1.0,0.25)); // white
        wrapVis->SetForceSolid(true);

        const G4double wIn  = hXY + wrapGap*mm;      // inner half-width
        const G4double wOut = wIn + wrapThk*mm;      // outer half-width

        if (wrapSides) {
            // A hollow square tube around the 4 long faces — the faces you
            // actually touch when handling the assembled module. The inner box
            // is over-long in z so the subtraction leaves an OPEN-ended tube
            // (the ends are the separate caps below).
            auto oS = new G4Box("wrapO", wOut, wOut, stackZ*mm/2);
            auto iS = new G4Box("wrapI", wIn,  wIn,  stackZ*mm/2 + 1.*mm);
            auto sS = new G4SubtractionSolid("WrapSides", oS, iS);
            auto sLV = new G4LogicalVolume(sS, tyvek, "WrapSides");
            sLV->SetVisAttributes(wrapVis);
            new G4LogicalSkinSurface("WrapSidesSurf", sLV, wrapSurf);
            new G4PVPlacement(nullptr, {0,0,0}, sLV, "WrapSides", worldLV, false, 0);
        }
        if (wrapEnds) {
            // End caps sit BEYOND the SiPMs (which occupy [front-2*pdHz, front]),
            // so they intercept only light that already missed a photodetector
            // and would otherwise be lost out the front/back face. Solid, no
            // holes: the fibres stop at the stack ends and never reach here.
            // The upstream cap is in the beam path but is only ~0.2 mm of
            // polyethylene (~5e-4 X0) — negligible as a preshower.
            const G4double capHz = wrapThk*mm/2;
            auto cS  = new G4Box("wrapCap", wOut, wOut, capHz);
            auto cLV = new G4LogicalVolume(cS, tyvek, "WrapEnd");
            cLV->SetVisAttributes(wrapVis);
            new G4LogicalSkinSurface("WrapEndSurf", cLV, wrapSurf);
            const G4double zUp = front - 2*pdHz - wrapGap*mm - capHz;
            const G4double zDn = back  + 2*pdHz + wrapGap*mm + capHz;
            new G4PVPlacement(nullptr, {0,0,zUp}, cLV, "WrapEnd", worldLV, false, 0);
            new G4PVPlacement(nullptr, {0,0,zDn}, cLV, "WrapEnd", worldLV, false, 1);
        }
    }
    G4cout << "[WRAP] outer wrap: sides=" << wrapSides << " ends=" << wrapEnds
           << "  R=" << wrapRefl
           << "  finish=" << (wrapSpecular ? "specular" : "diffuse")
           << "  gap=" << wrapGap << " mm  thick=" << wrapThk << " mm"
           << (wrapGap <= 0. && (wrapSides || wrapEnds)
                 ? "   [!] gap=0: wrap in optical contact, LYSO/air TIR destroyed" : "")
           << "  (RADWRAP_SIDES/_ENDS/_REFLECTIVITY/_FINISH/_GAP_MM/_THICK_MM)"
           << G4endl;
    ...
    G4cout << "[WRAP] stack " << stackZ << " mm, DSB1 centre z = " << dsbC/mm

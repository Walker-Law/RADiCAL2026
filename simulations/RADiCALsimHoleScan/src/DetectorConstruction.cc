...
G4VPhysicalVolume* DetectorConstruction::Construct() {
    ...
    // ─── HOLE-DIAMETER SCAN (RADiCALsimHoleScan) ─────────────────────────────
    // Study: light output at the capillary ENDS vs the diameter of the holes
    // drilled through the tiles. All five holes are set to a common diameter D
    // (RADICAL_HOLE_DIAM_MM, default 1.30 mm = nominal corner hole), and every
    // capillary SCALES TO FILL its hole (OD = D) with its internal features held
    // in proportion to the paper's nominal design ratios (arXiv:2401.01747):
    //     corner timing: OD 1150 µm, WLS fiber 900 µm  → fiber/OD = 0.7826
    //     center energy: OD 1000 µm, EJ309 bore 400 µm → bore /OD = 0.4000
    // Sweep D = 1.2 → 2.0 mm (0.1 mm steps) to map light output vs hole size.
    G4double holeDiam = 1.30*mm;
    if (const char* hd = std::getenv("RADICAL_HOLE_DIAM_MM")) {
        G4double v = std::atof(hd);
        if (v > 0.) holeDiam = v*mm;
    }
    const G4double holeR = holeDiam / 2.0;
    G4cout << "[RADiCAL] Hole-scan: all 5 tile holes D = " << holeDiam/mm
           << " mm; capillaries fill (OD = D)." << G4endl;

    // All five tile holes share this radius; capillaries fill them exactly.
    const G4double centerHoleR = holeR;
    const G4double cornerHoleR = holeR;

...
| **A1d** | **NEW — Cherenkov-cluster timing (D12c).** The first photon (pure Cherenkov) beats the CFD at 18 ps, and at true light the prompt cluster is ~10³ photons/end — enough to average down SPTR. A CFD computed *within* the prompt cluster might realizably recover ~20 ps or better. | **The only identified path back under ~20 ps without electronics changes.** | **Free** — offline on stored `phT`/`phWls`; no rerun. |
...
| **A3** | **SPTR (~60 ps single-photon jitter).** With the floor at ~28 ps and the quantile averaging ~N/20 photons, SPTR/√(N/20) is small at true light — but it decides whether A1d's Cherenkov-cluster idea (~10³ photons) actually works. | Decides the realizable version of A1d. | **Free** — smear `phT` offline and re-derive; no code change, no rerun. |

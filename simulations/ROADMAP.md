...
| **dispersion_check, 120 GeV, f=0.01** | 500 (+ 50 discarded) | on Mac — **DONE 08-16**: +17% shift versus the pre-dispersion rung, 1.1σ (Discovery 17); the 50-event first attempt was unusable and discarded, not reported |
| **fscan25, 25 GeV, f=0.01/0.1/0.5** | 500/500/250 | on Mac — **DONE 08-19**: timing-vs-f trend as expected, but the dispersion re-check contradicts Discovery 17 (Discovery 18) — Gap A9's effect is unconfirmed, not closed |
...
children below are now CLOSED; Gap A9/A9b (whether dispersion moves
anything, and if so how much) is the one open systematic standing
between "~28 ps" and a fully trusted number — genuinely open, not just
unmeasured (Discovery 17 versus Discovery 18).
...
| **Gap A9** | ~~No chromatic dispersion~~ **REBUILT, EFFECT UNCONFIRMED (2026-08-16/19).** Was: LYSO/DSB1 `RINDEX` flat → zero chromatic arrival-time spread. Now: rebuilt with Sellmeier curves. First check (Discovery 17, 120 GeV/f=0.01) suggested +17%, 1.1σ; a second independent check (Discovery 18, 25 GeV/f=0.01 and f=0.1) gave inconsistent-direction shifts and an unrelated quantity (energy resolution) shifted by a comparable amount — evidence the signal is smaller than sample-to-sample noise at 500-event statistics. | Whether dispersion measurably changes anything is now genuinely open, not just unmeasured. | **Rebuild done; the physics question is not.** Needs a paired same-seed comparison (Discovery 18), not more independent-seed statistics. |
| **Gap A9b** | **Does dispersion move the ~28 ps floor itself, and by how much?** Still fully open — now blocked behind Gap A9 itself being unresolved, not just behind reaching the floor regime. | The last open systematic standing between "~28 ps" and a fully trusted headline number. | Once Gap A9 is resolved with a paired-seed test, rerun at the floor regime (f≥0.5) the same way. |
...
|---|---|---|
| **<10 ps** | **NOT met by light alone, and now confirmed robust to that conclusion**: ~28–31 ps at 120 GeV (5% constant fraction discriminator, immune to single-photon time resolution smearing per Discovery 14); 35–37 ps at 25 GeV true light, and B *rises* toward lower energy (Discovery 15). The old "MET at 8.2 ps" was the first-photon artifact (Discovery 11); estimator, single-photon-time-resolution, and beam-energy fixes are all exhausted (Discovery 14, Discovery 15). Chromatic dispersion's effect is unresolved — one check suggested it adds spread (Discovery 17), a second, independent check contradicts that (Discovery 18) — so ~28 ps carries an unquantified, unconfirmed-direction systematic (Gap A9/A9b). | **Hardware, not analysis**: faster light path — prompt-light collection, thinner/faster wavelength-shifting material, geometry. The simulation's remaining role: settle whether dispersion matters at all via a paired-seed test (Gap A9), then test hardware variants (for example window length, wrap) under `dTcfd`. |

...
| **A1b** | ~~Is the 5% threshold safe?~~ **ANSWERED 2026-08-04 (D12): yes.** Prompt contamination below the 5% threshold is ≈4% at both 120 and 25 GeV — the CFD times on WLS, not Cherenkov. The transition zone is at 0.5–2%, and it is unstable there — do not lower the fraction into it. | Was the one question that could flip the timing story. | **Done** (`analysis/cfdfrac.C` on stored waveforms). |
| **A1c** | ~~Does B = 8.24 ps survive `dTcfd`?~~ **ANSWERED 2026-08-04 (D11): no.** True-light 120 GeV under the 5% CFD is ≈28 ps (three independent routes agree). 8.24 ps was the order-statistic artifact. | The headline number changed: light alone no longer meets <10 ps. | **Done** (CFD ladder + Run A). |
| **A1d** | ~~Cherenkov-cluster timing~~ **ANSWERED 2026-08-08 (D14): mostly no.** Best realizable variant (mean of first ~10 photons) gives 26 ps at 120 GeV and survives SPTR, but collapses to 140 ps at 25 GeV — energy-fragile, not a foundation. No estimator found below ~26 ps. | The last identified analysis-side path under ~20 ps is closed; <10 ps is now a hardware question. | **Done** (`analysis/sptr_est.C`). |
| **A2** | ~~B(E)?~~ **ANSWERED 2026-08-09 (D15): B falls with energy, shallowly.** 25 GeV B = 32.8 ± 2.4 ps vs 120 GeV B = 20.4 ± 5.8 ps; B ~ E^−0.15…−0.30. The gap's own stated expectation (B grows with E) was backwards. | **Energy is not a lever** — closing 28→10 ps would need ~3500 GeV, and real running is below 120 GeV, i.e. worse. | **Done** (7-rung 25 GeV ladder). |
| **A3** | ~~SPTR (~60 ps single-photon jitter)~~ **ANSWERED 2026-08-08 (D14): harmless to the CFD.** Even 100 ps SPTR shifts `dTcfd` by ~1 ps (quantile of ~10³–10⁴ photons). The light-side numbers stand unchanged in front of a real SiPM. | The largest feared electronics term costs ~nothing at the trigger level. | **Done** (`analysis/sptr_est.C`). |
...
| **A9** | **No chromatic dispersion (found 2026-08-08).** LYSO/DSB1 `RINDEX` was flat → Geant4's derived group velocity = c/n for every wavelength: in-crystal light ~7% too fast (real n_g(420) ≈ 1.95 vs 1.82) and ZERO chromatic arrival-time spread. Biases B low by an unmeasured few-ps-scale amount. | The floor numbers (D11) carry this as a systematic until re-run. | **Source ready** (16-pt Sellmeier curves, SIMPLE + Wrap, syntax-checked). Rebuild + 1-rung A/B (f=1e-2, 120 GeV, ~40 min local) once the running ladder finishes. |
...
|---|---|---|
| **<10 ps** | **NOT met by light alone, and now confirmed robust to that conclusion**: ~28–31 ps at 120 GeV (5% CFD, SPTR-immune per D14); 35–37 ps at 25 GeV true light, and B *rises* toward lower energy (D15). The old "MET at 8.2 ps" was the first-photon artifact (D11); estimator, SPTR, and beam-energy fixes are all exhausted (D14, D15). | **Hardware, not analysis**: faster light path — prompt-light collection, thinner/faster WLS, geometry. Sim's remaining role: dispersion re-check (A9) and testing hardware variants (e.g. window length, wrap) under `dTcfd`. |

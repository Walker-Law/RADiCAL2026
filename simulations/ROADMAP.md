...
| **A2** | **B(E) — partially answered.** 25 GeV true light = 34.5 ± 2.2 ps, but f=0.3→1.0 was still improving steeply, so that is an upper bound on the 25 GeV floor, not the floor itself. The 25 GeV ladder never got its turnover under `dTcfd`. | Determines whether ~28 ps is one number or a curve. | Low-f 25 GeV rungs exist only in dTwls-era data; a 25 GeV CFD mini-ladder if it ever matters. |

...
describe_tag() {
  case "$1" in
    ...
    *paperR_lad*)   echo "REALISTIC-COMPOSITION SCALE-LADDER point: paperR base (paper183 geometry, coherent thinning of ALL light sources, PDE(lambda), waveform-realism electronics chain), light factor f in the tag. Fit a^2=A^2/f+B^2 via analysis/ladder_paperR.C and extrapolate to true light (f=100)." ;;
    *_paperR)       echo "REALISTIC-COMPOSITION RUN: paper183 geometry + SCINT_YIELD thinned coherently with LYSO_SCINT_SCALE and QUARTZ_CHER_KEEP (self-scint 70%->~0% of timing light, physical WLS-dominated mix) + MicroFJ PDE(lambda). First run: 500 evt, MAX_OPT_PHOTONS=20e6 (the 1000-evt paperR predecessor was budget-truncated at 125/150 GeV and deleted). NOT directly comparable to the paper at thinned light — needs the ladder extrapolation." ;;

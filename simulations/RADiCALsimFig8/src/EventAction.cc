...
void EventAction::EndOfEventAction(const G4Event*) {
    ...
    // Fig-8 single-ended study: per-corner energy-weighted fiber SIGNAL time —
    // the reference that strips shower-depth/time jitter so the downstream σ_t is
    // PURE photostatistics (like Fig 8, which shows a clean 1/√LY with no floor).
    std::array<G4double,4> fiberTime; fiberTime.fill(kBigTime);
    for (G4int c = 0; c < 4; c++)
        if (cornerEdepSum[c] > 0.) fiberTime[c] = cornerTSum[c] / cornerEdepSum[c];

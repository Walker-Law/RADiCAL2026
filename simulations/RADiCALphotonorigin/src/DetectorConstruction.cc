...
G4VPhysicalVolume* DetectorConstruction::Construct() {
    ...
    // Create and place one per-corner capillary segment. Each corner gets a
    // distinct volume name ("<base>_<corner>") so trajectories can be coloured by
    // origin corner, while the copy number stays = corner index for scoring.
    auto placeCornerSegment = [&](G4VSolid* solid, G4Material* material,
                                  G4VisAttributes* vis, const G4String& baseName,
                                  G4int corner, const G4ThreeVector& position) {
        auto lv = new G4LogicalVolume(solid, material,
                                      baseName + "_" + std::to_string(corner));
        lv->SetVisAttributes(vis);
        new G4PVPlacement(nullptr, position, lv, baseName + "_Phys",
                          logicCalo, false, corner);
    };

    for (G4int corner = 0; corner < 4; corner++) {
        const G4ThreeVector xy = capXY[corner + 1];   // capXY[0] is the centre hole
        placeCornerSegment(solidTUpstream,   quartz, tRodVis, "Cap_Corner_Upstream",   corner, xy + G4ThreeVector(0, 0, z_upstream));
        placeCornerSegment(solidTMidTube,    quartz, tRodVis, "Cap_Corner_MidTube",    corner, xy + G4ThreeVector(0, 0, z_wls));
        placeCornerSegment(solidTMidWLS,     luag,   wlsVis,  "Cap_Corner_WLS",        corner, xy + G4ThreeVector(0, 0, z_wls));
        placeCornerSegment(solidTDownstream, quartz, tRodVis, "Cap_Corner_Downstream", corner, xy + G4ThreeVector(0, 0, z_downstream));

        new G4PVPlacement(nullptr, xy + G4ThreeVector(0, 0, zPDUpstream),
                          logicPDUpstream, "PDUpstream_Phys", logicCalo, false, corner);
        new G4PVPlacement(nullptr, xy + G4ThreeVector(0, 0, zPDDownstream),
                          logicPDDownstream, "PDDownstream_Phys", logicCalo, false, corner);
    }
